...
function NBR_cron_job_worker($hardcron=false)
{
    ...
    if (is_array($rows_for_balance_check)) {
        ...
        foreach ($rows_for_balance_check as $row_for_balance_check) {
            ...
            $balance_info_array = NBR__getreceivedbyaddress_info($address_request_array, $krbwc_settings);
            ...
            WHERE `id`='$row_id';";
                ...
                if ($balance_info_array['balance'] > 0) {
                    NBR__log_event(__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['krb_address']}, Payment ID = {$row_for_balance_check['krb_payment_id']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");

                    if ($balance_info_array['balance'] < $last_order_info['order_total']) {
                        NBR__log_event(__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['krb_address']}, Payment ID = {$row_for_balance_check['krb_payment_id']}' (KRB '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
                    }
                }
                ...
                if ($balance_info_array['balance'] >= $last_order_info['order_total']) {
                    ...
                    */
                    ...
                    NBR__log_event(__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['krb_address']}, Payment ID = {$row_for_balance_check['krb_payment_id']}' (KRB '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
                ...
                NBR__log_event(__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['krb_address']} Payment ID: {$row_for_balance_check['krb_payment_id']}: " . $balance_info_array['message']);
            }
            //..//
        }
    }
